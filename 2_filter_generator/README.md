...
2. [Low-pass to Band-pass conversion](#low-pass-to-band-pass-conversion)
	...
	2. [Variable change and component calculations](#variable-change-and-components-calculation)
...
4. the desired cutoff frequencies of the Band-pass filter
...
3. The center frequency and the relative bandwidth of the Band-pass
...
The resulting Band-pass filter will have the same passing band length as the original Low-pass. Thus, given a **cutoff frequency** $f_{LP}$ and calling the **two cutoff frequencies of the Band-pass** $f_{BP_r}$ (the higher one, consequently on the right) and $f_{BP_l}$ the smaller one we'll have
...
Conversely, studying the capacitors' conductances one finds that
